...
; FontCharSet := 1          ; 字符集
; TextColor := "Auto"       ; 文本颜色
; TextBkColor := "Auto"     ; 文本背景颜色
WordWrap := False       ; 自动换行
AutoURL := False        ; 自动检测 URL
Zoom := "100 %"         ; 缩放比例
ShowWysiwyg := False    ; 显示所见即所得
CurrentLine := 0        ; 当前行
CurrentLineCount := 0   ; 当前行数
...
; 主 GUI ==============================================================================================================
GuiNum := 1 ; GUI 编号
MainGui := Gui("+ReSize +MinSize", GuiTitle) ; 创建主 GUI
...
MainGui.MarginX := MarginX                   ; 设置水平边距
MainGui.MarginY := MarginY                   ; 设置垂直边距
; 样式按钮----------------------------------------------------------------------------------
...
MainT1 := MainGui.AddText("x+10 yp hp", "WWWWWWWW") ; 添加文本
MainT1.GetPos(&TX := 0, &TY := 0, &TW := 0, &TH := 0) ; 获取文本位置和大小
TX := EditW - TW + MarginX ; 计算新的 X 坐标
MainT1.Move(TX) ; 移动文本
MainGui.SetFont("Norm", "Arial") ; 恢复正常字体
Options := "x" . TX . " y" . TY . " w" . TW . " h" . TH ; 设置选项
...
; 对齐和行间距 --------------------------------------------------------------------------------------------------------
MainGui.SetFont("Norm", "Arial") ; 设置正常字体
MainGui.AddText("0x1000 xm y+2 h2 w" . EditW) ; 添加分隔线
...
Options := "xm y+5 w" . EditW . " r20" ; 设置选项
...
RE2.GetPos( , , &REW, &REH) ; 获取 RichEdit 控件大小
...
; 测试 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
^+1:: { ; 热键 ^+1
   RC := Buffer(16, 0) ; 创建缓冲区
   DllCall("SendMessage", "Ptr", RE2.HWND, "UInt", 0x00B2, "Ptr", 0, "Ptr", RC.Ptr) ; EM_GETRECT 获取矩形
   CharIndex := DllCall("SendMessage", "Ptr", RE2.HWND, "UInt", 0x00D7, "Ptr", 0, "Ptr", RC.Ptr, "Ptr") ; EM_CHARFROMPOS 获取字符索引
   LineIndex := DllCall("SendMessage", "Ptr", RE2.HWND, "UInt", 0x0436, "Ptr", 0, "Ptr", Charindex, "Ptr") ; EM_EXLINEFROMCHAR 获取行索引
   MsgBox("First visible line = " . LineIndex) ; 显示第一行可见行
}
^+f:: {
   CS := RE2.GetSel()
   SP := RE2.GetScrollPos()
...
;    CF2 := RE2.GetCharFormat()                   ; 获取当前选择的 CHARFORMAT2 对象
   CF2.Mask := 0x40000001                       ; CFM_COLOR = 0x40000000, CFM_BOLD = 0x00000001
   CF2.TextColor := 0xFF0000                    ; 颜色格式为 BGR
   CF2.Effects := 0x01                          ; CFE_BOLD := 0x00000001
   RE2.SetSel(0, 0)                             ; 从文本开头开始搜索
   While (RE2.FindText("Lorem", ["Down"]) != 0) ; 查找特定短语
      RE2.SetCharFormat(CF2)                    ; 应用新的设置
   CF2 := ""
...
; 所见即所得
WYSIWYGFN(Item, *) {
   Global ShowWysiwyg ^= True ; 切换所见即所得状态
